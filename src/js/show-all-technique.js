let list = document.querySelector('.types-of-technique__list');
let show_or_hide_button = document.querySelector('.types-of-technique__list + .button-show .button-show__button');
...
show_or_hide_button.addEventListener('click', function () {
  if (!showed) {
      ...
      list.classList.add('types-of-technique__list--height-auto')
      showed = true;
  } else {
      show_or_hide_button.textContent = "Показать всё";
      ...
      list.classList.remove('types-of-technique__list--height-auto')
      showed = false;
  }
});
